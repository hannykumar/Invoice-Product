...
export class AuthenticationService {
  ...
  authenticate(sessionId: Id): RequestContext { const session = this.#sessions.get(sessionId); if (!session || session.revokedAt || session.expiresAt <= this.now()) throw new PlatformError("SESSION_EXPIRED", "SESSION_EXPIRED: Sign in again."); return this.access.context(session.companyId, session.branchId, session.userId, session.id); }
