...
import type { AuditEvent, ApprovalPolicy, CommandRecord, CommandStatus, Id, Permission, RequestContext } from "./types.ts";
...
export class AuditLog {
  ...
  append(event: Omit<AuditEvent, "id" | "occurredAt">): AuditEvent {
    const stored = { ...event, id: randomUUID(), occurredAt: new Date().toISOString(), before: event.before && redact(event.before), after: event.after && redact(event.after) };
    this.#events.push(Object.freeze(stored));
    return stored;
  }
  forCompany(context: RequestContext): readonly AuditEvent[] { return this.#events.filter((event) => event.companyId === context.companyId); }
...
export class PlatformCommandService {
  ...
  create(context: RequestContext, input: Omit<CommandRecord, "id" | "companyId" | "branchId" | "actorId" | "status" | "createdAt">): CommandRecord {
    ...
    const record: CommandRecord = { ...input, id: randomUUID(), companyId: context.companyId, branchId: context.branchId, actorId: context.actorId, status: "draft", createdAt: new Date().toISOString() };
    this.#commands.set(record.id, Object.freeze(record)); this.#idempotency.set(key, { commandId: record.id, payload });
  ...
  transition(context: RequestContext, id: Id, next: CommandStatus, reason?: string): CommandRecord {
    ...
    const updated = Object.freeze({ ...current, status: next }); this.#commands.set(id, updated);
