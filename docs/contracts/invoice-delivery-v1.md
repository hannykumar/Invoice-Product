...
Email and WhatsApp receive a PDF attachment from the company-scoped `InvoicePdfSource` along with the document URL. If PDF generation fails, the provider is not called and the attempt can be retried. SMS keeps the document URL without an attachment. Preferences can opt a recipient out by channel. Provider outcomes are appended as queued, sent, delivered, bounced, failed and retry-queued events. Failed attempts can be retried in place, retaining the audit history.
