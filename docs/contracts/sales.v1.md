...
interface SalesPolicy {
  series: { prefix, branchCode };               // INV/26-27/000042
...
of the number altogether; the default series is `INV/26-27/000001`.

**Sixteen characters, and all sixteen of them (v1.2.0, issue #162).** CGST Rule 46(b) asks for "a
consecutive serial number not exceeding sixteen characters", and the e-invoice portal refuses a
document number longer than that. So:
...
- The financial year is written short — `26-27`, not `2026-27`. The long form spent seven of the
  ...
  characters long.
- **The width of the running number is not configurable.** `NumberSeries` has no `padding` field.
  The sequence is padded to exactly the room the prefix and year leave behind, so a number is always
  sixteen characters and never fifteen. A character not spent on the running number is an order of
  magnitude of headroom given away, and it cannot be reclaimed later without renumbering bills that
  are already in customers' hands.
- `sequenceDigits(series)` and `billsPerYear(series)` report the derived width and the resulting
  ceiling. The default carries a business to **999,999 bills in one financial year**.
- `validateSeries` runs in the `SalesService` constructor and refuses a series that could not reach
  `MIN_BILLS_PER_YEAR` (99,999 — what this product managed before the widening, so no choice can
  leave a business worse off than it already was). What is checked is whether the prefix and branch
  code have eaten too much of the number, not whether the business guessed its volume correctly.
...
Numbers already issued under the old format stay as they are; nothing rewrites history. Consumers
must change in two ways:

- `NumberSeries` no longer takes `padding`. Remove the field; the width is derived.
- Anything that pattern-matched `INV/{branch}/{4-digit year}/{5 digits}`, or that read the financial
  year out of the third segment of the number, must be updated — `financialYear` on the invoice is
  the field to read.
...
| `SALES_SERIES_TOO_FEW_BILLS` | The prefix and branch code leave too little room; fewer than 99,999 bills a year |
| `SALES_SERIES_EXHAUSTED` | Every number this series can issue this financial year has been used |
