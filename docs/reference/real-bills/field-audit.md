...
## The e-way bill page (#191)

The third real document in this folder, `eway-bill-291.pdf`, is a page printed from the government
e-way bill portal, not a document a business designs. So it is not held to our layout at all: it is
copied, section by section, heading by heading, in the portal's own words. An officer at a check
post reads those headings to find a field, and a better-looking arrangement of them is a worse
document.

**The page is a convention, not a requirement.** CGST Rule 138A(1) asks the person in charge of the
vehicle to carry the invoice, bill of supply or delivery challan, and *either* a copy of the e-way
bill in physical form *or* the e-way bill number in electronic form. The number alone is enough in
law. Nothing in the product is blocked, held or refused because this page was not printed; it exists
because every driver in India carries it and every check post expects it.

| Section on `eway-bill-291.pdf` | On ours |
| --- | --- |
| E-Way Bill Details — number, generated date, generated by, valid upto, mode, approx distance, type, document details, transaction type | all nine, from what the portal answered |
| Address Details — From (GSTIN, legal name, state code, Dispatch From with PIN) and To (the same, with Ship To) | both, with the dispatch-from and ship-to addresses kept apart from the billing ones |
| Goods Details — HSN, product name and description, quantity with unit, taxable amount, tax rate as `C+S+I+Cess+Cess Non.Advol`, then the eight totals | all of it; the non-advalorem cess, which we do not hold, prints as `0.00` rather than being dropped from a five-part string |
| Transportation Details — transporter ID and name, transporter document number and date | printed when a transporter was named; empty labels print, as they do on the real page |
| Vehicle Details — mode, vehicle or transport document number and date, from, entered date, entered by, consolidated e-way bill number, multi-vehicle info | a row per vehicle leg, oldest first |

**What ours does not carry, and why**

- **No QR code.** The portal's page has one. Ours draws it only if a provider's response gives us
  its content, and no provider we are connected to does. Nothing stands in its place — not an empty
  box, not a placeholder. A QR square we composed ourselves would scan to something the government
  never said.
- **No copy marking.** The portal's page carries none, so neither does ours. Rule 48's
  ORIGINAL / DUPLICATE / TRIPLICATE copies are prescribed for a tax invoice.
- **No logo, mark of the trade or colours.** This is a government form, and a business that brands
  it has made its own document out of the portal's.
- **Multi-vehicle info is blank**, because nothing in the product records it yet.

**What the page says when the e-way bill is not simply travelling**

| State | What prints |
| --- | --- |
| Part A only, no vehicle | a heavy line under the header: `PART-B NOT ENTERED — the goods may not move on this e-way bill yet`, and "Not started" where the validity goes |
| Cancelled | `CANCELLED on <date>`, and the button that prints it says "Print (cancelled)" |
| Expired | `EXPIRED at <the moment it ran out>` |
| Rejected by the other party | `REJECTED by the other party on <date>` |
| Failed, or never raised | no page at all; the screen says why, and the offline Part A file keeps its own note saying it is not an e-way bill |
