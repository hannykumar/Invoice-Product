...
## The driver's e-way bill (#191)

`eway-bill-291.pdf` is the benchmark: the page the government portal prints, which a driver is
handed with the invoice. Ours is held to it section by section. The headings are the portal's own
words and are not renamed, because those are the words an officer reads.

| Portal section | Fields on the real page | On ours |
| --- | --- | --- |
| E-way Bill Details | number, generated date, generated by, valid upto, mode, approximate distance, type, document details, transaction type | all nine; the date and time in Indian wall-clock as `DD/MM/YYYY hh:mm AM/PM` |
| Address Details | From and To: GSTIN, legal name, state, plus Dispatch From and Ship To with PIN | all of them; when the goods dispatch from or ship to a different address, that address prints and the transaction type says so |
| Goods Details | HSN, product name and description, quantity with unit, taxable amount, `C+S+I+Cess+Cess Non.Advol`, then the eight totals | all of them; amounts print plain with two decimals, as the portal prints them |
| Transportation Details | transporter ID and name, transporter document number and date | all four; each says "Not recorded" rather than printing an empty box |
| Vehicle Details | mode, vehicle or transport document number and date, from, entered date, entered by, consolidated e-way bill number, multi-vehicle info | all of them, one row per leg the consignment has travelled on |

What the real page has and ours does not:

- **The portal's QR code.** Our e-way bill provider's response carries no QR content, and the code
  on the real page encodes the portal's own signed string. Drawing a square of our own on a
  government document would be a forgery, so the page carries no QR and no placeholder box.
- **Multi-vehicle information.** The column prints, and is empty: the product does not yet split one
  consignment across several vehicles under one e-way bill.

Deliberate differences: one copy and no copy markings, because the portal's page carries none.
A page is refused outright until the portal has given a number. A bill with no vehicle on it yet
prints under `PART-B NOT ENTERED — the goods may not move on this e-way bill yet`; a cancelled one
prints `CANCELLED on <date>` and an expired one `EXPIRED at <time>`, because the paper exists and a
person may still have to produce it. Nothing in the product is blocked for want of this page:
Rule 138A(1) accepts the number carried electronically, and printing it is a convention.
